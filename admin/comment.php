...
$selectcomment=mysqli_query($connect,"SELECT * FROM comment WHERE c_active = '2' ");
if(mysqli_num_rows($selectcomment) == 0){
	...
	echo'
		...
		';
		$selectCom=mysqli_query($connect,"SELECT * FROM comment WHERE c_active = '2' ");
      while($row=mysqli_fetch_assoc($selectCom)){
		...
		echo '
		 ...
		 <td>
			<a href="comment.php?delete=comment&c_id='.$row['c_id'].'" class="btn btn-danger">حذف</a>
